...
| `plan-tasks-implement` | Partial | First-pass plan -> tasks shell -> implement path, flags, resume boundaries | Node dispatches this dedicated workflow through the pipeline runner without a legacy-alias note. The narrow planning runtime and tasks shell feed the supported batch implementation path; real task decomposition, per-task/wave, and full resume transitions remain gaps. |
| `plan-implement` | Partial | First-pass plan -> implement path, flags, resume boundaries | Node dispatches this dedicated workflow through the pipeline runner without a legacy-alias note. The narrow planning runtime feeds the supported batch implementation path; planning revision/adversarial/signoff loops, per-task/wave, and full resume transitions remain gaps. |
| `tasks-implement` | Partial | Tasks shell -> implement path, file/existing plan behavior, flags, resume boundaries | Node dispatches this dedicated workflow through the pipeline runner without a legacy-alias note. A plan file or existing `plan.md` can feed the tasks shell and supported batch implementation path; real task decomposition, per-task/wave, and full resume transitions remain gaps. |
